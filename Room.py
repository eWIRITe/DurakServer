from Enums import Status, Role, status, LastMove, GameType
...
class Room:
    def __init__(self, rid, json, comission=0.1):
        ...
        self.m_key = json["key"]
        self.m_gType = GameType.value = json["type"]
    ...
    async def start(self):
        ...
        for player in self.m_players:
            await self.m_writeOffChips(player.get_uid(), (self.m_bet / len(self.m_players)) * -1)
    ...
    async def finish(self):
        ...
        for player in self.m_players:
            await self.m_writeOffChips(player.get_uid(), self.m_bet / len(self.m_players))
    ...
    async def end_turn(self, last_move):
        print("The end of turn")

        main_player = self.get_mainTurnPlayer()

        if last_move == last_move.grabbing:
            main_player.get_next().role = Role.firstThrower
            main_player.get_next().status = status.throwing

            main_player.role = Role.main
            main_player.status = status.beating

        elif last_move == last_move.folding:
            main_player.role = Role.firstThrower
            main_player.status = status.throwing
    ...
    async def throw(self, sid, card, UserID):
        player = self.get_player(UserID)

        if not player.has_card(card):
            print("ERROR: Player:" + UserID + ", dont has card:" + card)
            return False
        ...
        if player.get_RoleEnum() == Role.firstThrower:
            if len(self.m_battlefield) == 0:
                ...
                await self.m_throwCallback(self.get_sidOfAllPlayers(), card, UserID, sid)
            else:
                if self.can_throw_card(card):
                    self.m_battlefield.attack(card)
                    await self.m_throwCallback(self.get_sidOfAllPlayers(), card, UserID, sid)
        else:
            if self.m_gType == GameType.ThrowIn:
                if len(self.m_battlefield) > 0:
                    if self.can_throw_card(card):
                        self.m_battlefield.attack(card)
                        await self.m_throwCallback(self.get_sidOfAllPlayers(), card, UserID, sid)
    ...
    async def battle(self, sid, uid, atackedCard, atackingCard):
        player = self.get_player(uid)

        # check data
        if not player:
            print("ERROR: Player:" + uid + ", is not exist")
            return False
        if not player.is_playing():
            print("ERROR: Player:" + uid + ", is not playing")
            return False
        if not player.has_card(atackingCard):
            print("ERROR: Player:" + uid + ", dont has card:" + atackingCard)
            return False

        if player.get_RoleEnum() == Role.main:
            print("beating")
            self.m_battlefield.beat(atackedCard, atackingCard, self.m_trump)
            player.m_cards.remove(atackingCard)
        else:
            print("not beaten")

        if atackedCard.get_nominal().value == atackingCard.get_nominal().value and self.m_gType == GameType.Transferable:
            await self.transfer()

        await self.m_beatCallback(sid, self.m_rid, uid, atackedCard, atackingCard)
    ...
    async def grab(self):
        ...
        await self.end_turn(LastMove.grabbing)
    ...
    async def cl_fold(self, uid):
        ...
        print("Cards in the battlefield: " + str(len(self.m_battlefield)))
    ...
    async def fold(self):
        ...
        await self.end_turn(LastMove.folding)
    ...
    async def distribute(self, uid):
        ...
        if not player.is_playing() or player.get_num_cards() >= 6:
            print("User cards number: " + uid + " - " + str(player.get_num_cards()))
            print("Player cards: \n" + str(player.get_cards()))
            return []
        if len(self.m_mainDeck) == 0:
            return
